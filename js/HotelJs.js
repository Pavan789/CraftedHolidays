...
$(document).ready(function () {
    ...
    $('#hotelSearch').click(function () {
        ...
        if (HotelValidations()) {
            ...
            if ($('#isDomesticHotel').val() == "true") {
                locationString = "<b>We are searching for best hotels in  " + $('#city').val() + "</b>";
            }
            else {
                var w = $('#destCity :selected').index();
                var selected_text = $('#destCity :selected').text();
                locationString = "<b>We are searching for best hotels in  " + selected_text + "</b>";
            }
            logoimage = $('#Logo').html();
            ...
            logoimage = $('#Logo').html();
            loaderimage = $('#load-img').html();
            $('#HotelForm').attr("action", "http://booking.craftedholidays.com/HotelSearchResult.aspx");
            $('#HotelForm').submit();
           
		   $('#HotelForm').append($('<div/>',  {
                id: 'holdy',
				class:'wgtpopbox',
                html: loading
            }));
    ...
    ////**********************Calender related js***********************************************//
    var min = '0';
    $("#checkInDate").datepicker({ minDate: min, maxDate: '+1Y', selectedDate: '0', dateFormat: 'dd/mm/yy', numberOfMonths: 2, closeText: '',
        onSelect: function () {
        },
    ...
    $('#city').keydown(function (e) {
        ...
        $(this).autocomplete({
            source: function (request, response) {
                $.ajax({
                    type: "POST",
                    url: "HotelCityList.php",
                    data: "Type=HotelSearchDomestic&requestFrom=HotelSearchDomestic&searchKey=" + request.term,
